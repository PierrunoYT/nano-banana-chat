import { Button } from '@/components/ui/button';

interface PoweredByBannerProps {
  onOpenSettings?: () => void;
}

export default function PoweredByBanner({ onOpenSettings }: PoweredByBannerProps) {
  return (
    <div className="bg-yellow-500 text-white text-sm md:text-lg py-2 relative flex items-center justify-center">
      <div>
        <a
          href="https://replicate.com/google/nano-banana?utm_source=project&utm_campaign=kontext-chat"
          target="_blank"
          rel="noopener noreferrer"
          className="underline"
        >
          Powered by Nano-Banana on Replicate & FAL
        </a>
      </div>
      <Button
        variant="ghost"
        size="icon"
        onClick={onOpenSettings || (() => {})}
        className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-yellow-400 hover:bg-yellow-300 text-yellow-800 rounded-full transition-all duration-200"
        title="API Settings"
        style={{ display: onOpenSettings ? 'flex' : 'none' }}
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
        </svg>
      </Button>
    </div>
  );
}